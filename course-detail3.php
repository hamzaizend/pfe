<?php 
 session_start();
    ...
    $req = $db->query('SELECT id FROM stories');
    ...
    $sql = 'SELECT * FROM stories ORDER BY id LIMIT '.$offset.' , '.$nbre_articles_par_page.' ';
    ...
    if($last_page != 1){
        if($page_num > 1){
            $previous .= $page_num-1;
            $pagination .= '<a href="course-detail3.php?page='.$previous.'">Précédent</a>';

            for($i=$page_num-$nbre_pages_max_gauche_et_droite; $i<$page_num; $i++){
                if($i > 0){
                    $pagination .= '<a href="course-detail3.php?page='.$i.'">'.$i.'</a>';
                }
            }
        }
        ...
        if($page_num != $last_page){
            $next .= $page_num + 1;
            $pagination .= '<a href="course-detail3.php?page='.$next.'">Suivant</a> ';
        }
    }
