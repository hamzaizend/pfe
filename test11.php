...
$requete= $bdd->prepare('SELECT id FROM test WHERE id_user=? and num=? and etat=?');
$requete->execute(array($_SESSION['id'],10,'Test Reussi'));
$ok=$requete->rowCount();
if(!$ok){
    $_SESSION['flash']['danger']="Vous devez d'abbord passé et reussir le 2eme test de la section General english";
    header('Location: course-general-english.php');
    exit();
}
...
$score=0;
          ...
          if(isset($_POST['submit'])) {
            ...
            if($score == 8){
              ...
              $insert->execute(array($_SESSION['id'],11,'Test Reussi'));
              $delete->execute(array('Test Pas Reussi',$_SESSION['id'],11));
              ...
              exit();
                ...
                $res_fail = "EMM Dommage , vous pouvez réessayer ";
                  ...
                  $find->execute(array($_SESSION['id'],11,'Test Pas Reussi'));
                  $ok=$find->rowCount();
                if(!$ok){ 
                  $insert=$bdd->prepare("INSERT INTO test(id_user,num,etat) VALUES(?,?,?)");
                  $insert->execute(array($_SESSION['id'],11,'Test Pas Reussi'));
                ...
                $updatereq->execute(array($_SESSION['id'],11));
                $_SESSION['flash']['danger']=$res_fail ." <br> ".$resultat;
                }
              }
    ...
    <section class="course_details_area section_gap bg-light text-center">
        ...
        <div class="container border border-dark rounded p-2">
            <div class="text-center lead ">
                ...
                <h2 class="lead "> * Give the right order for the events bellow ( 1 , 2 , 3 ...ect)* </h2>
                </p>
                <br />
                <form method="POST" action="">
                    <p> <input type="text" name="text1" value="<?php if(isset($text1)){
  echo $text1; }?>" class="check003" size="1" /><text class="button002"></text> Henry changes what he usually thinks
                        about .<?php if(isset($resulat1)) echo $resultat1; ?><br> <br>

                        <input id="input002" size="1" name="text2" value="<?php if(isset($text2)){
  echo $text2; }?>" /><text class="button002" id="check002"></text>Henry thinks he sees the beginning of a new star .
                        <?php if(isset($resulat2)) echo $resultat2; ?><br><br>

                        <input id="input003" size="1" name="text3" value="<?php if(isset($text3)){
  echo $text3; }?>" /> <text class="button002" id="check003"></text>They see an image of the baby for the first time
                        .<?php if(isset($resulat3)) echo $resultat3; ?><br><br>

                        <input id="input004" size="1" value="<?php if(isset($text4)){
  echo $text4; }?>" name="text4" /><text class="button002" id="check004"></text>The baby is born .
                        <?php if(isset($resulat4)) echo $resultat4; ?><br><br>

                        <input id="input005" value="<?php if(isset($text5)){
  echo $text5; }?>" size="1" name="text5" /><text class="button002" id="check005"></text> They decide on a name for the
                        baby .<?php if(isset($resulat5)) echo $resultat5; ?><br><br>


                        <input type="text" value="<?php if(isset($text6)){
  echo $text6; }?>" name="text6" id="input006" size="1" /><text class="button002" id="check006"></text> Henry feels
                        really worried about his new responsibillities
                        .<?php if(isset($resulat6)) echo $resultat6; ?><br>
                        <br>

                        <input type="text" value="<?php if(isset($text7)){
  echo $text7; }?>" name="text7" id="input007" size="1" /><text class="button002" id="check007"></text> Anna gives
                        Henry some news .<?php if(isset($resulat7)) echo $resultat7; ?><br><br>

                        <input id="input008" name="text8" size="1" value="<?php if(isset($text8)){
  echo $text8; }?>" /><text class="button002" id="check008"></text> The baby moves inside Anna
                        .<?php if(isset($resulat8)) echo $resultat8; ?>
                    ...
                    <div id="disappear001">
                        <div id="center001"><button class="btn btn-outline-dark pl-5 pr-5" name="submit">Finish</button>
