<?php 
 session_start();
    ...
    if($last_page != 1){
        if($page_num > 1){
            $previous .= $page_num-1;
            $pagination .= '<a href="course-detail.php?page='.$previous.'">Précédent</a>';

            for($i=$page_num-$nbre_pages_max_gauche_et_droite; $i<$page_num; $i++){
                if($i > 0){
                    $pagination .= '<a href="course-detail.php?page='.$i.'">'.$i.'</a>';
                }
            }
        }
        ...
        if($page_num != $last_page){
            $next .= $page_num + 1;
            $pagination .= '<a href="course-detail.php?page='.$next.'">Suivant</a> ';
        }
    }
